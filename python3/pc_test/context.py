...
class RuntimeContext:
    ...
    def _format_title_line(self, rc: int, no: str, text: str) -> tuple[str, str]:
        """Return (colored, plain) title lines without trailing newline."""
        if rc == TEST_RUNNING:
            colored = f"{self.CLR_WARN}Running{self.CLR_NORM}... {self.CLR_WARN}{no}. {self.CLR_LC2}{text}{self.CLR_NORM}"
            plain = f"Running... {no}. {text}"
            return colored, plain
        if rc == TEST_PASSED:
            status = f"[{self.CLR_OK}PASSED{self.CLR_NORM}  ] "
            plain_status = "[PASSED  ]"
        elif rc == TEST_SKIPPED:
            status = f"[{self.CLR_WARN}SKIPPED{self.CLR_NORM} ] "
            plain_status = "[SKIPPED ]"
        elif rc == TEST_BLOCKED:
            status = f"[{self.CLR_ERR}BLOCKED{self.CLR_NORM} ] "
            plain_status = "[BLOCKED ]"
        else:
            status = f"[{self.CLR_ERR}FAILED{self.CLR_NORM}  ] "
            plain_status = "[FAILED  ]"
        colored = f"{status}{self.CLR_WARN}{no}. {self.CLR_LC2}{text}{self.CLR_NORM}"
        plain = f"{plain_status} {no}. {text}"
        return colored, plain

    def draw_title_line(self, rc: int, no: str, text: str, *, log: bool | None = None) -> None:
        if log is None:
            log = rc != TEST_RUNNING
        colored, plain = self._format_title_line(rc, no, text)
