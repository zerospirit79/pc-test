...
from pc_test.context import FatalError, RuntimeContext
...
def sync_detected_hardware(ctx: RuntimeContext) -> None:
    ...
    if ctx.drives.strip() and not _test_enabled(ctx, "fio"):
        ctx.fio_test = "1"
    if ctx.have_xorg and os.environ.get("DISPLAY") and not _test_enabled(ctx, "v3d"):
...
def run_config_form(ctx: RuntimeContext) -> None:
    ...
    sync_detected_hardware(ctx)
    if os.environ.get("DISPLAY") and ctx.has_binary("yad"):
        run_gui(ctx)
